const getAuth = async () => {
  ...
  const clientSecret = "ea9a5056afba47da965b1c2cff8b1471";
...
const getData = async (x, y) => {
  ...
  let data = await fetch(
    `https://api.spotify.com/v1/search?type=track&q=track:${y}+artist:${x}&limit=1`,
    {
      method: "GET",
      headers: {
...
let saveSongDB = () => {
  document.getElementById("grid_artist").value =
    music.tracks.items[0].artists[0].name;
  document.getElementById("grid_track").value = music.tracks.items[0].name;
  document.getElementById("grid_img").value =
    music.tracks.items[0].album.images[0].url;
};
